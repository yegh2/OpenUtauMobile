...
1. 在最新一次 [Actions](https://github.com/yegh2/OpenUtauMobile/actions/workflows/build-unsigned-ipa.yml) 构建中下载未签名 IPA（artifact 名为 `OpenUtauMobile-unsigned-ipa`），解压得到 `.ipa` 后自行签名安装到 iPhone。
...
2. 构建完成后，下载 `OpenUtauMobile-unsigned-ipa` artifact，解压得到其中的 `.ipa`。
